    ...
    win32wm::WM_PAINT => {
      ...
      if subclass_input.event_loop_runner.handling_events() {
        if subclass_input.event_loop_runner.should_buffer() {
          ...
          // `flush_paint_messages` returns false when it is already running
          // (see its docs). The upstream assert assumed this WM_PAINT could
          // never re-enter because the flush skips *this* HWND.
          //
          // That is false once an IME is in the picture. CJK IMEs (especially
          // a Chinese IME on a Japanese Windows locale) attach candidate /
          // composition windows on focus and click; those HWNDs pump nested
          // messages, which re-enters this callback and trips the assert.
          // Release builds use panic=abort, so the assert kills the process
          // (tao#1140 / tauri#14088). Honor the re-entrant return instead.
